...
export default class Guest extends React.Component {
	...
	componentDidMount = async () => {
		...
		if(method !== 'undefined'){
			this.setState({ loading: true })
				 const data = await this.load(method)
  ...
  load = async (id) => {
	...
	fetchOrder = async (id) => {
 	...
 	submitOrder = async (fields) => {
		...
		if(guestOrder.response === 200){
			...
			window.location.href = '/guest/orders/'+fields._id
		}
		else {
	...
	render(){
		...
		return [
		    <Navbar/>,
        typeof method === 'undefined' && <Section className={`flex-fill px-5 align-items-center d-flex`} Name={`hero`} BackgroundColor={`#ffffff`} BackgroundImage={backgroundImage} BackgroundSize={`contain`} BackgroundPosition={`25% center`} Height={`50rem`}>
				...
				typeof method !== 'undefined' && [
					<Header className={`d-flex justify-content-center my-0 pt-5`} OverlineText={`Guest`} OverlineClassName={`text-muted`} TitleText={`Order`} Height={`15rem`}/>,
					<Section>
